...
extension MarketDatabase {
    func playbackQueue() throws -> [PlaybackQueueEntry] {
    ...
    @discardableResult
    func moveQueueItem(_ itemID: UUID, to requestedPosition: Int) throws
        -> [PlaybackQueueEntry]
    {
        try databaseQueue.write { database in
            var identifiers = try Self.queueItemIDs(in: database)
    ...
    @discardableResult
    func removeQueueItem(_ itemID: UUID) throws -> [PlaybackQueueEntry] {
        try databaseQueue.write { database in
            try database.execute(
    ...
    func clearPlaybackQueue() throws {
        try databaseQueue.write { database in
    ...
    func activateQueueItem(_ itemID: UUID) throws -> AudioItem {
        try databaseQueue.write { database in
