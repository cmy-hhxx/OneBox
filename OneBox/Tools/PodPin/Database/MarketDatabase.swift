...
public enum MarketDatabaseError: Error, Equatable, Sendable {
...
public struct ItemCursor: Equatable, Hashable, Sendable {
    ...
    /// last-played time.
    public let sortValueMilliseconds: Int64
    public let itemID: UUID

    public init(sortValueMilliseconds: Int64, itemID: UUID) {
        self.sortValueMilliseconds = sortValueMilliseconds
        self.itemID = itemID
    }

    /// Compatibility initializer for existing folder-page callers.
    public init(importedAtMilliseconds: Int64, itemID: UUID) {
        self.init(sortValueMilliseconds: importedAtMilliseconds, itemID: itemID)
    }

    public var importedAtMilliseconds: Int64 { sortValueMilliseconds }
}

public struct ItemPage: Equatable, Sendable {
    public let items: [AudioItem]
    public let nextCursor: ItemCursor?

    public init(items: [AudioItem], nextCursor: ItemCursor?) {
        self.items = items
...
public struct ArchiveRequest: Sendable {
    ...
    public init(items: [AudioItem], destinationFolderID: UUID) {
        self.items = items
        self.destinationFolderID = destinationFolderID
    }
}

public enum ArchiveItemResult: Equatable, Sendable {
...
public struct ArchiveResult: Equatable, Sendable {
    public let destinationFolderID: UUID
    public let itemResults: [ArchiveItemResult]

    public init(destinationFolderID: UUID, itemResults: [ArchiveItemResult]) {
        self.destinationFolderID = destinationFolderID
...
public actor MarketDatabase {
    public static let defaultFileName = "podpin.sqlite"
    public static let applicationSupportFolderName = "PodPin"
    ...
    public static func inMemory() throws -> MarketDatabase {
        try MarketDatabase(
    ...
    public static func open(atPath path: String) throws -> MarketDatabase {
        try MarketDatabase(
    ...
    public static func open(at url: URL) throws -> MarketDatabase {
    ...
    public func inbox() throws -> LibraryFolder {
    ...
    public func folder(id: UUID) throws -> LibraryFolder? {
        try databaseQueue.read { database in
            try Self.fetchFolder(id: id, in: database)
        }
    }

    public func listFolders(parentID: UUID? = nil) throws -> [LibraryFolder] {
        try databaseQueue.read { database in
            if let parentID {
    ...
    public func allFolders() throws -> [LibraryFolder] {
        try databaseQueue.read { database in
    ...
    @discardableResult
    public func createFolder(named name: String, parentID: UUID? = nil) throws -> LibraryFolder {
    ...
    @discardableResult
    public func renameFolder(_ id: UUID, to name: String) throws -> LibraryFolder {
        let normalizedName = try Self.normalizedFolderName(name)
    ...
    @discardableResult
    public func moveFolder(_ id: UUID, toParentID parentID: UUID?) throws -> LibraryFolder {
        try databaseQueue.write { database in
            guard var folder = try Self.fetchFolder(id: id, in: database) else {
                throw MarketDatabaseError.folderNotFound(id)
            }
    ...
    public func item(id: UUID) throws -> AudioItem? {
        try databaseQueue.read { database in
            try Self.fetchItem(id: id, in: database)
        }
    }

    public func item(platform: AudioPlatform, contentID: String) throws -> AudioItem? {
    ...
    public func listItems(in folderID: UUID = LibraryFolder.inboxID) throws -> [AudioItem] {
        try databaseQueue.read { database in
    ...
    public func listAllItems() throws -> [AudioItem] {
        try databaseQueue.read { database in
    ...
    public func recoverInterruptedDownloads() throws -> [UUID] {
        try databaseQueue.write { database in
    ...
    @discardableResult
    public func insertItem(_ item: AudioItem) throws -> AudioItem {
    ...
    @discardableResult
    public func archive(_ request: ArchiveRequest) throws -> ArchiveResult {
    ...
    @discardableResult
    public func refreshMetadata(_ item: AudioItem) throws -> AudioItem {
        let normalizedItem = try Self.validated(item)
        return try databaseQueue.write { database in
            guard
                let existing = try Self.fetchItem(
    ...
    @discardableResult
    public func updateItem(_ item: AudioItem) throws -> AudioItem {
        let normalizedItem = try Self.validated(item)
        return try databaseQueue.write { database in
    ...
    @discardableResult
    public func moveItem(_ id: UUID, toFolderID folderID: UUID) throws -> AudioItem {
        try databaseQueue.write { database in
            guard var item = try Self.fetchItem(id: id, in: database) else {
                throw MarketDatabaseError.itemNotFound(id)
            }
    ...
    public func currentPlaybackItem() throws -> AudioItem? {
        try databaseQueue.read { database in
            guard
    ...
    public func setCurrentPlaybackItem(_ id: UUID?) throws {
        try databaseQueue.write { database in
