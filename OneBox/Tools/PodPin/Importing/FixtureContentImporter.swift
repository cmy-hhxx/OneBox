...
/// Deterministic importer used by the first runnable slice and unit tests.
/// It is intentionally opt-in: production URLs still flow to the platform adapter.
private final class PodPinResourceBundleToken: NSObject {}
...
actor FixtureContentImporter: ContentImporting {
    ...
    static func bundled() throws -> FixtureContentImporter {
        guard
            let audioURL = Bundle(for: PodPinResourceBundleToken.self).url(
                forResource: "podpin-sample",
                withExtension: "m4a"
            )
        else {
            throw ContentImportError.mediaUnavailable("示例音频没有随应用安装。")
        }
...
actor PodPinContentImporter: ContentImporting {
    ...
    init(
        ...
        self.platformImporter =
            platformImporter ?? YTDLPContentImporter(anonymousSession: anonymousSession)
        ...
        self.mediaMaterializer = mediaMaterializer ?? MediaMaterializer()
    }
