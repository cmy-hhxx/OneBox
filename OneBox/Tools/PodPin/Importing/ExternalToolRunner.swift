...
struct BundledToolLocator: Sendable {
    ...
    init(bundle: Bundle = .main, overrides: [String: URL] = [:]) {
        self.bundle = bundle
    ...
    func executableURL(named name: String) throws -> URL {
        ...
        if let url = bundle.url(forResource: name, withExtension: nil, subdirectory: "Tools") {
            return url
        }

        #if DEBUG
            let environmentKey =
                "PODPIN_\(name.uppercased().replacingOccurrences(of: "-", with: "_"))_PATH"
