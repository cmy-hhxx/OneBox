...
final class PodPinStore: ObservableObject {
    ...
    init(
        ...
        #if DEBUG
            let bundledFixture: FixtureContentImporter?
            let fixtureFailure: ContentImportError?
            do {
                bundledFixture = try FixtureContentImporter.bundled()
                fixtureFailure = nil
                ...
                bundledFixture = nil
                fixtureFailure = .mediaUnavailable(error.localizedDescription)
            }
            let debugImporter: any ContentImporting =
                importer
                ?? PodPinContentImporter(
                    fixture: bundledFixture,
                    fixtureFailure: fixtureFailure
                )
        #else
            let debugImporter: any ContentImporting = importer ?? PodPinContentImporter()
        #endif
        self.importer = debugImporter
