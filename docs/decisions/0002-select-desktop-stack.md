...
股票看盘和博客收听主要需要网络、持久化、通知与音频；晃动窗口聚焦需要较深的桌面窗口和辅助功能集成。OneBox v1 的支持范围确定为 macOS，并且没有现有代码需要迁移。
...
- 宿主骨架已经完成。下一步先按 README 的决策门槛选定一个工具，再使用该技术栈完成最小端到端业务竖切；ADR 不替代产品优先级选择。
