# ADR-0008: 使用 OneBox 命名空间的 GRDB 数据库并一次性导入独立应用数据
...
- Xcode 当前不会把 GRDBSQLite 的 Clang module map 传播给 `StockWatchTool` 静态库。`project.yml` 因此只给该 target 设置指向 `$(BUILD_DIR)/../../SourcePackages/checkouts/GRDB.swift/Sources/GRDBSQLite/module.modulemap` 的 `OTHER_SWIFT_FLAGS`；标准 check 和 benchmark DerivedData 布局必须覆盖这个 workaround，工具链不再需要时应删除。
