...
- 本 ADR 接受时，Xcode 不会把 GRDBSQLite 的 Clang module map 传播给 `StockWatchTool` 静态库，因此股票 target 曾使用局部 `OTHER_SWIFT_FLAGS`。ADR-0010 将 StockWatch 迁入独立 package 后，manifest 直接依赖 GRDB 与 GRDBSQLite products，并删除股票路径的 DerivedData 相对 workaround；PodPin 当时在完成同类迁移前仍保留必要设置，随后也改由自身 manifest 直接依赖两个 GRDB products，并删除了根工程 workaround。
