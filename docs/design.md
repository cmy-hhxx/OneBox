...
- 行情行按名称/代码/交易所、分时、最新价/涨跌幅形成稳定列；任意数量通过垂直滚动承载，不为长列表增加分页或独立窗口。数字使用等宽数字。
- 检查器默认展开，连续排列“所选标的、价格提醒、数据”而不是分成多套导航。内容净宽至少 700pt 时右侧 dock 为 248pt；更窄时从右侧覆盖显示，宽度在 320pt 与 360pt 之间，不能继续压缩行情列。
