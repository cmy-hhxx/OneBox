...
report_path="$artifact_root/Logs/podpin-benchmark-$run_id.json"
rerun_log_path="$artifact_root/Logs/podpin-benchmark-$run_id-rerun.log"
rerun_report_path="$artifact_root/Logs/podpin-benchmark-$run_id-rerun.json"
configured_baseline=${ONEBOX_PODPIN_BENCHMARK_BASELINE:-}
conventional_baseline="$repository_root/Benchmarks/Baselines/podpin.json"
...
if [[ -n "$configured_baseline" && ! -f "$configured_baseline" ]]; then
    print -u2 "Configured PodPin benchmark baseline does not exist: $configured_baseline"
    exit 2
fi
...
if [[ -n "$baseline_path" ]]; then
    "$script_directory/compare-benchmark-report.sh" "$baseline_path" "$baseline_path" >/dev/null || {
        print -u2 "Configured PodPin benchmark baseline is invalid: $baseline_path"
        exit 2
    }
    jq -e '.benchmark == "PodPin"' "$baseline_path" >/dev/null || {
        print -u2 "Configured baseline is not a PodPin benchmark report: $baseline_path"
        exit 2
    }
fi
...
run_benchmark() {
    ...
    "$script_directory/write-benchmark-report.sh" "PodPin" "$destination_log" "$destination_report" || report_status=$?
...
if [[ -z "$baseline_path" ]]; then
    print "No PodPin benchmark baseline configured; relative regression comparison skipped."
    print "Set ONEBOX_PODPIN_BENCHMARK_BASELINE or add $conventional_baseline to enable it."
fi
...
if evaluate_benchmark "$log_path" "$report_path"; then
    ...
    print -u2 "PodPin benchmark first run failed ($first_failure_kind, exit $first_failure_status); rerunning the full benchmark once."
    rerun_performed=true

    if evaluate_benchmark "$rerun_log_path" "$rerun_report_path"; then
        print "PodPin benchmark rerun passed; the first failure is treated as noise."
    else
        evaluation_status=$?
        ...
        print -u2 "PodPin benchmark failed on both full runs."
